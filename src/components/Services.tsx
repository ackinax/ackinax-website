...
function statusColor(status: ServiceCard["status"]) {
  if (status === "LIVE") return { text: "hsl(160 60% 45%)", bg: "hsl(160 60% 45% / 0.1)", border: "hsl(160 60% 45% / 0.3)" };
  if (status === "ACTIVE") return { text: "hsl(29 88% 67%)", bg: "hsl(29 88% 67% / 0.1)", border: "hsl(29 88% 67% / 0.3)" };
  return { text: "hsl(215 17% 42%)", bg: "hsl(215 17% 42% / 0.1)", border: "hsl(215 17% 42% / 0.3)" };
}
...
function Card({ card, index }: { card: ServiceCard; index: number }) {
  ...
  return (
    <div
      ref={ref}
      className={`card-base relative overflow-hidden transition-all duration-500 ${
        isVisible ? "animate-fade-slide-up" : "opacity-0"
      } ${isSoon ? "opacity-50 cursor-default" : "group cursor-pointer"}`}
    ...
    >
      ...
      <div className={`${!isSoon ? "group-hover:bg-card-hover" : ""} absolute inset-0 rounded-xl transition-colors duration-300`} />

      <div className="relative z-10">
        <div className="flex items-start justify-between mb-5">
          <span className="text-[28px] leading-none">{card.icon}</span>
          <span
            className="font-mono-brand text-[10px] font-medium tracking-wider px-2.5 py-1 rounded-full border"
